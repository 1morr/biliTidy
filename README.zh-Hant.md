...
Bilibili 給你收藏夾，卻不幫你判斷哪支影片該進哪一個，所以積了幾百支的都永遠留在預設收藏夾；它給你關注清單，卻看不出誰已經不再投稿，所以清單只會越來越長。biliTidy 替你做判斷、把確認留給你：用瀏覽器裡既有的登入態讀你的
B 站資料，攤成一張每一列都帶著證據的審核表，讓你勾選、確認、批次執行、可撤銷。在你按下按鈕之前，什麼都不會被搬動、沒有人會被取關。
...
- **這一輪可以下載成 CSV**：每一列都寫出去，AI 建議了什麼與你最後選了什麼各佔一欄，還有理由與它用了哪些欄位。
...
biliTidy 由同一位作者更早的兩個 private 擴充功能合併而成：biliFavOrg（收藏夾）與 biliFollowCleaner（關注）。它們做的是同一種事，所以現在共用一個工具列圖示、一份設定、一把限速器、同一套外觀；合併後的歷史在這個 repo 繼續。

...
圖示一律是自己畫的 SVG（`ui/components/icons.tsx`），16 格線稿、`currentColor`、圓端圓角接合。品牌記號是一個帶 28 圓角的深色方塊，裡面三條飄過的彈幕——由上到下黃、粉、藍，長短與起點錯開，像正在飛過畫面——世界的縮圖：介面的彈幕色板直接變成記號。這是粉色唯一不表示「現在／你選的」的地方，因為記號不在介面裡，它就是介面的縮圖；三條的粗細（16 單位）是為了同一份 SVG 縮到 16px 時仍是三條看得見的色線。
